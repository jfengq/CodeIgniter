...
class Zui_Xin_Model extends CI_Model {
    ...
    {
        $query = $this->db->query('select * from zuixin where code ="' . $code . '"');
        return $query->row();
    }
    ...
    public function update_stock_list ($arr, $code) {
        if (count($arr) > 0) {
            ...
            if (count($arr) < 1) {
                $this->db->insert('zuixin', $stock);
            } else {
                $this->db->query('update zuixin set day0=' . $zuixin . ' where code="' . $code . '"');
            }
        }
    }
    ...
    {
        $query = $this->db->query('select * from zuixin');
        return $query->result_array();
    }
    ...
    public function back_up ()
    {
        ...
        foreach ($query->result() as $row)
        {
            ...
            $day2 = $row->day2;
            $day3 = $row->day3;
            $this->db->query('update zuixin set day0=0, day1=' . $day0 . ', day2=' . $day1 . ', day3=' . $day2 . ', day4=' . $day3 . ' where code="' . $code . '"');
        }
    }
}
